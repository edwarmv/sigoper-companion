...
const transition = (
  state: typeof initialScanSessionState,
  ...actions: Parameters<typeof scanSessionReducer>[1][]
...
describe("scan-session boundary", () => {
  it("starts idle and walks the phone connection lifecycle", () => {
    ...
    expect(sessionStatusLabel(state.status)).toBe(
      "Phone paired — ready to scan",
    );
  });
  ...
  it("allows incomplete review but requires explicit acknowledgement to finish", () => {
    ...
    expect(
      scanSessionReducer(incomplete, { type: "finishReview" }).status,
    ).toBe("reviewingIncomplete");
    expect(
      scanSessionReducer(incomplete, {
        type: "finishReview",
  ...
  it("confirms destructive transitions and does not discard on dismissal", () => {
    ...
    const confirming = scanSessionReducer(review, {
      type: "requestNewSession",
    });

    expect(confirming.status).toBe("confirmingNewSession");
    expect(
      scanSessionReducer(confirming, { type: "dismissConfirmation" }),
    ).toMatchObject({
      status: "reviewingComplete",
      rawPayload: "payload",
    });
    expect(
      scanSessionReducer(confirming, { type: "confirmDestructiveAction" }),
    ).toEqual(initialScanSessionState);
  });
  ...
  it("keeps review and failure available when the phone disconnects", () => {
    ...
    expect(
      scanSessionReducer(review, { type: "phoneDisconnected" }).status,
    ).toBe("reviewingComplete");
    ...
    expect(
      scanSessionReducer(failed, { type: "phoneDisconnected" }).status,
    ).toBe("processingFailed");
  });

  it("rejects submissions unless the phone is paired and prevents replacement after review", () => {
    const idleSubmission = scanSessionReducer(initialScanSessionState, {
      type: "receiveScan",
      rawPayload: "bad",
    });
